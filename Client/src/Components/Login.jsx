...
const Login = ({ setUserName, setUserEmail }) => {
  ...
  const handleAuth = async (e) => {
    ...
    if (!isLogin && !validatePassword(password)) {
      toast.error("Password must be at least 6 characters, with an uppercase letter, a number, and a symbol.", { autoClose: 2000 });
      return;
    }
    ...
    try {
      ...
      const userName = isLogin ? (user.displayName || email.split("@")[0]) : signupName;
      ...
      if (error.code === "auth/email-already-in-use") {
        toast.error("Email already in use. Try another email.", { autoClose: 2000 });
        ...
        toast.error("Authentication failed. Please try again.", { autoClose: 1000 });
      }
    }
  };
      ...
      <div className="p-4 relative bg-[url('images/wall1.webp')] bg-cover bg-center w-full h-[85vh] flex items-center justify-center">
        <div className="bg-black bg-opacity-70 p-8 md:p-12 rounded-xl shadow-lg w-full max-w-xl">
          ...
          <form onSubmit={handleAuth} className="space-y-4 mt-4">
            ...
            <div className="text-center">
              <button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-white py-2 px-6 rounded-full transition duration-300">
